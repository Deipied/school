...
public class LinkedStackOfStrings { // public class LinkedStackOfStrings
    ...
    if(temp.item.equals(item)) return true; // if the item is in the
   stack
    ...
    // prints the term exits in the stack
    } else { // otherwise
    System.out.println(str + " does not exist in the linked stack");
    // java prints that the term is not in the stack
    }
   // You could add another method to the class to iterate (and print) the items in
   the list.
   // And then call it via an object of the class in the main method. The method
